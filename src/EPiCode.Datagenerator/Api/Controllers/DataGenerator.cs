    ...
    public class DataGeneratorController : ApiController
    {
        ...
        public List<Address> CreateCustomers(int count)
        {
            ...
            foreach (Address address in addresses)
            {
                ...
                EPiCode.Datagenerator.Api.DataObjects.Address newAddress = new EPiCode.Datagenerator.Api.DataObjects.Address
                    {
                        Name = "Home",
        ...
        [HttpPost]
        public string CompleteOrders([FromBody] List<int> orders)
        {
            Stopwatch tmr = Stopwatch.StartNew();

            foreach (var orderId in orders)
            {
             var order =  OrderContext.Current.GetPurchaseOrderById(orderId);

                OrderStatusManager.ReleaseOrderShipment(order.OrderForms[0].Shipments[0]);
                OrderStatusManager.CompleteOrderShipment(order.OrderForms[0].Shipments[0]);

                order.AcceptChanges();

                OrderGroupWorkflowManager.RunWorkflow(order, OrderGroupWorkflowManager.OrderCompleteShipmentWorflowName);
            ...
            tmr.Stop();
            return string.Format("{0} orders completed in {1}",orders.Count,tmr.Elapsed );
        }
